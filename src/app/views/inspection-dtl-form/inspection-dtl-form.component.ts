...
export class InspectionDtlFormComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onAddRecommendationsQuick(recommendationType, typee, recType, recDetail, id, subitem1) {
    let len = new Date().getTime();
    let typeeLower = (<string>typee).toLowerCase();
    let fileName = 'rec-file-' + typeeLower + '-' + len;
    let itemValue = this.getRecommendationItemValue(recommendationType);
  ...
  showRecommendations(event, recommId, recommType_short, id, itemType, itemValue, recommType, typee, subitem1) {
    this.recommQuickAddCurrentCheckItem = { id: id, recType: itemValue, recommType_short: recommType_short, recDetails: itemType, recommType: recommType, typee: typee, recommId: recommId, subitem1: subitem1 };
    ...
    if (!this.hasRecommendationAddedForItem(recommId)) {
      this.onAddRecommendationsQuick(recommType, typee, this.recommQuickAddCurrentCheckItem.recType, this.recommQuickAddCurrentCheckItem.recDetails, this.recommQuickAddCurrentCheckItem.id, this.recommQuickAddCurrentCheckItem.subitem1);
    }
